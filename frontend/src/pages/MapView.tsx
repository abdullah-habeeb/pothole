...
const MapUpdater = ({ center }: { center: [number, number] }) => {
  const map = useMap();
  const lastCenterRef = useRef<string>(`${center[0]},${center[1]}`);

  useEffect(() => {
    const newCenterStr = `${center[0]},${center[1]}`;
    if (lastCenterRef.current !== newCenterStr) {
      // Only fly to new center if it has significantly changed or if it's the very first load
      // But here we simply rely on the center prop changing (which happens when new potholes are added at top of list)
      map.flyTo(center, map.getZoom(), { duration: 1.5 });
      lastCenterRef.current = newCenterStr;
    }
  }, [center, map]);
...
const SelectionFocusController = ({ focus }: { focus: { lat: number; lng: number } | null }) => {
  ...
  useEffect(() => {
    if (focus) {
      map.flyTo([focus.lat, focus.lng], Math.max(map.getZoom(), 16), { duration: 1.0 });
    }
  }, [focus, map]);
  return null;
};
...
const MapView = () => {
  ...
  const { data: contractorsData } = useQuery({
    ...
    queryFn: authApi.getContractors,
    // Only fetch if government or admin (which implies isAuthorized) - or just always fetch if authorized
    enabled: user?.role === 'government' || user?.role === 'admin',
  });
  ...
  const potholes = ensureArray(apiPotholes, []) as Pothole[];
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${mapType === 'osm'
              ...
              <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${mapType === 'satellite'
                  ? 'bg-primary-600 text-white'
                  : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
              >
                    ...
                    <div className="space-y-3">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          // Use selected contractor ID as the name or ID field?
                          // The 'createAssignment' API expects 'contractorName' currently (string).
                          // But we want to link by ID.
                          // The backend model was updated to use 'contractorId' (User ObjectId).
                          // The API service might still send 'contractorName'.
                          // I should check `contractorAssignmentApi.createAssignment` signature.
                          // For now, I'll send the ID as 'contractorName' if the backend expects ID in that field
                          // OR I need to update the frontend service to send 'contractorId'.
                          // Let's assume for now I send the ID.
                        ...
                        disabled={!contractorName || createAssignmentMutation.isPending || selectedCount === 0}
