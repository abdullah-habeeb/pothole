...
const UploadVideo = () => {
  ...
  const handleAnalyze = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Increase timeout to 5 minutes (300000 ms) as video processing can be slow
      ...
      // AUTO-SAVE: Automatically save to backend if confident
      if (detections.length > 0) {
        toast.info('Auto-saving detections to map...');
        setSaving(true);
        try {
          const payload = {
            severity: resultWithSeverity.severity || 'none',
            previewImage: resultWithSeverity.preview_image,
            potholes: detections,
            gps: gpsPoints,
          };
          const saveResponse = await potholeApi.saveDetections(payload);
          setBackendResult(saveResponse);
          toast.success(`Successfully saved ${saveResponse.video.pothole_count} potholes to Map!`);
          queryClient.invalidateQueries({ queryKey: ['potholes'] });
        } catch (saveError) {
          console.error(saveError);
          toast.error('Analysis done, but failed to save to map. Please try "Save" button.');
        } finally {
          setSaving(false);
        }
      } else {
        toast.info('No potholes to save.');
      }
      ...
      if (error instanceof DOMException && error.name === 'AbortError') {
        toast.error('The request timed out. The video is taking too long to process. Please try a shorter video.');
      } else {
        toast.error(
          errorMessage || 'Unable to reach the FastAPI server. Is it running on port 8000?'
        );
      }
      setMlResult(null);
    } finally {
