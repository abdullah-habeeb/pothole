...
const Assignments = () => {
  ...
  const { data: contractorsData } = useQuery({
    ...
    enabled: isAuthorized && (user?.role === 'government' || user?.role === 'admin'),
  });
      ...
      <div className="bg-white shadow rounded-lg p-6">
          ...
          <div className="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div className="flex gap-3 items-end">
              ...
              <div className="text-sm text-gray-600">
                {selection?.items?.length || 0} pothole{(selection?.items?.length || 0) !== 1 ? 's' : ''} selected
              </div>
              <button
                onClick={handleCreateAssignment}
                disabled={!contractorId || createAssignmentMutation.isPending}
        ...
        <div className="flex border-b border-gray-200 mb-6">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeList === 'assigned'
          ...
          <button
            ...
            className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeList === 'notAssigned'
              ? 'border-primary-600 text-primary-600'
              : 'border-transparent text-gray-500 hover:text-gray-700'
              }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeList === 'fixed'
              ? 'border-primary-600 text-primary-600'
              : 'border-transparent text-gray-500 hover:text-gray-700'
              }`}
          >
                ...
                {assigned.map((assignment) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        {/* 
                            TODO: We now have contractorId in assignment or populated data?
                            The API response type for ContractorAssignment has contractorName. 
                            We should ensure backend populates it or returns the name.
                            For now, assuming backend returns 'contractorName' (which might be the ID if not populated)
                         */}
                        <p className="font-semibold text-gray-900">{assignment.contractorName || 'Unknown Contractor'}</p>
