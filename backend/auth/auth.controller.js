...
// @access  Public
const ADMIN_PASSKEY = 'admin123'; // Hardcoded as per requirements
...
export const signup = async (req, res) => {
  try {
    const { name, email, password, role, contractorId, governmentId, passkey } = req.body;
    ...
    // Role validation
    const validRoles = ['citizen', 'government', 'contractor', 'admin'];
    ...
    if (userRole === 'government' && !governmentId) {
      return res.status(400).json({
        success: false,
    ...
    const user = new User({
      ...
      governmentId: userRole === 'government' ? governmentId : undefined,
      isGovernmentAuthorized: false, // Default to false
    });

    await user.save();

    // If Government User, create Pending Authorization Request
    if (userRole === 'government') {
      // Dynamic import to avoid circular dependency issues if any, though likely safe
      const GovAuthorizationRequest = (await import('../models/GovAuthorizationRequest.js')).default;

      await GovAuthorizationRequest.create({
        userId: user._id,
        email: user.email,
        status: 'pending',
      });
    ...
    // For Admin, we can auto-authorize specialized permissions if needed, 
    // but the role check handles it. 
...
export const getContractors = async (req, res) => {
  try {
    const contractors = await User.find({ role: 'contractor' })
      .select('name email contractorId')
      .sort({ name: 1 });
