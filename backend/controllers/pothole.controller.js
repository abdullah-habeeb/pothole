...
const matchGpsPoint = (detection, gpsPoints, totalDetections) => {
  ...
  console.log(`[GPS-MATCH] Fallback to index: ${clampedIndex} / ${gpsPoints.length} -> ${fallbackPoint.latitude},${fallbackPoint.longitude}`);
...
export const uploadPotholes = async (req, res) => {
  try {
    ...
    console.log(`[UPLOAD] Request received. Potholes: ${potholes?.length}, GPS: ${gps?.length}`);
    ...
    if (!detections.length) {
      console.log('[UPLOAD] No detections found in request.');
    ...
    console.log(`[UPLOAD] Processing ${detections.length} detections with ${gpsPoints.length} GPS points.`);

    // Process and save potholes synchronously to ensure data integrity
    try {
      const savedPotholes = await processPotholesInBackground(videoId, severity, previewImage, gpsPoints, detections);
      console.log(`[UPLOAD] Saved ${savedPotholes.length} potholes to DB.`);
      ...
      const responseData = {
        ...
        video: {
          ...
          pothole_count: savedPotholes.length,
        ...
        potholes: savedPotholes.map((pothole) => ({
          ...
          latitude: pothole.latitude,
          longitude: pothole.longitude,
          previewImage: pothole.previewImage,
          confidence: pothole.confidence,
          gpsMatch: {
            source: pothole.gpsMatch?.source || 'none',
          },
        })),
        message: `Successfully saved ${savedPotholes.length} potholes.`,
      };

      res.status(201).json(responseData);
    } catch (saveError) {
      console.error('Failed to save potholes:', saveError);
      return res.status(500).json({
        success: false,
        message: 'Failed to save pothole data to database.',
